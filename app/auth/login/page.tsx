...
export default function LoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (email === 'admin@d2d.com' && password === 'admin123') {
      // Admin login - redirect to admin dashboard
      localStorage.setItem('user', JSON.stringify({ email, role: 'admin', name: 'Admin User' }))
      router.push('/admin')
    } else if (email === 'operator@d2d.com' && password === 'operator123') {
      // Operator login - redirect to admin dashboard
      localStorage.setItem('user', JSON.stringify({ email, role: 'operator', name: 'Tour Operator' }))
      router.push('/admin')
    } else if (email === 'user@example.com' && password === 'user123') {
      // Regular user login - redirect to home
      localStorage.setItem('user', JSON.stringify({ email, role: 'user', name: 'Test User' }))
      router.push('/')
    } else {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center bg-sand-light px-4 py-12">
        <div className="max-w-md w-full">
          <div className="bg-white rounded-lg shadow-xl p-8">
            <h1 className="text-3xl font-bold text-center mb-8">Sign In to D2D</h1>
